...
@main def javalibFunctionLongSpecialized(): Unit =
  var consumerTotal = 0L
  val longConsumer: LongConsumer = value => consumerTotal += value
  longConsumer.andThen(value => consumerTotal += value * 10L).accept(3L)
  ...
  val longBinary: LongBinaryOperator = (left, right) => left * right
  val toLong: ToLongFunction[String] = value => value.length.toLong
  val toLongBi: ToLongBiFunction[String, String] = (left, right) => (left.length + right.length).toLong

  println("long-consumer:" + consumerTotal)
  ...
  println("long-unary:" + longUnary.compose(value => value * 3L).andThen(value => value - 1L).applyAsLong(4L))
