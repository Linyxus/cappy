...
private[util] object RedBlackTree {
  ...
  private def minNodeAfter[A, B](node: Node[A, B], key: A, boundKind: BoundKind)(
    ...
    if (node eq null) {
      ...
      var current = node
      var result: Node[A, B] = null
      var done = false
      while (!done) {
        val cmp = compare(key, current.key)
        if (cmp == 0) {
          result = if (boundKind == InclusiveBound) current else successor(current)
          done = true
        } else if (cmp < 0) {
          val child = current.left
          if (child eq null) {
            result = current
            done = true
          } else {
  ...
  private def maxNodeBefore[A, B](node: Node[A, B], key: A, boundKind: BoundKind)(
    ...
    if (node eq null) {
      ...
      while (!done) {
        val cmp = compare(key, current.key)
        if (cmp == 0) {
          result = if (boundKind == InclusiveBound) current else predecessor(current)
          done = true
        } else if (cmp < 0) {
          val child = current.left
          if (child eq null) {
  ...
  private[this] def fixAfterInsert[A, B](tree: Tree[A, B],
    ...
    while !done do
      ...
      else if isBlack(parent) then
        done = true
      else
        val grandParent = parent.parent
        if parent.isLeftChild then
  ...
  private[this] def successor[A, B](node: Node[A, B]): Node[A, B] = {
    if (node.right ne null) {
      ...
      while (!done) {
        val parent = current.parent
        if ((parent eq null) || (current eq parent.left)) {
          result = parent
          done = true
        } else {
          current = parent
  ...
  private[this] def predecessor[A, B](node: Node[A, B]): Node[A, B] = {
    if (node.left ne null) {
      ...
      while (!done) {
        val parent = current.parent
        if ((parent eq null) || (current eq parent.right)) {
          result = parent
          done = true
        } else {
          current = parent
