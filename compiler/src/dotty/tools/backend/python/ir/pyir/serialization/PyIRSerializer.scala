...
object PyIRSerializer:
  ...
  private final class Serializer:
    ...
    private def treeTag(t: PyTree): Byte = t match
      ...
      case _: PyDictValue       => TagPyDictValue
      case _: PyListValue       => TagPyListValue
      case _: PyRawTupleValue   => TagPyRawTupleValue
    ...
    private def writeTreePayload(t: PyTree): Unit = t match
      ...
      case n: PyDictValue =>
        writeType(n.tpe)
        writeNat(n.entries.size)
        n.entries.foreach { case (k, v) =>
          writeTree(k)
          writeTree(v)
        }
