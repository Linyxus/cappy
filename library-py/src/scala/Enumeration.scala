...
abstract class Enumeration (initial: Int) extends Serializable {
  ...
  /** Python-backend variant of `populateNameMap`.
   *
   *  Upstream Scala uses Java reflection (`getClass.getMethods` +
   *  `Method.invoke`) to recover the user-declared `val Foo = Value`
   *  names — both pieces of which the Python runtime does not
   *  implement.
   *
   *  Codegen on the Python backend stores user-declared vals as plain
   *  Python instance attributes: `val Red = Value` lowers to
   *  `self.Red = Value()`. So a one-pass walk of `vars(self)` recovers
   *  the same name → Value pairs that the JVM gets via reflection,
   *  without touching `Class.getMethods` / `Method.invoke`.
   */
  private def populateNameMap(): Unit = {
    val attrs = PyBuiltins.instance_attrs(this)
    val n = attrs.length
    var i = 0
    while (i < n) {
      attrs.value(i) match {
        case v: Value if v.outerEnum eq thisenum =>
          nmap += ((v.id, attrs.name(i)))
        case _ =>
      }
      i += 1
    }
