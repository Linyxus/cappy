...
private[java] object IntFloatBits {
  // Inlined subset of java.util.internal.GenericArrayOps — delete when
  // the L4 port lands.
  sealed trait ArrayOps[T] {
    def get(array: Array[T], index: Int): T
    def set(array: Array[T], index: Int, value: T): Unit
  }

  sealed trait ArrayCreateOps[T] {
    def create(length: Int): Array[T]
  }

  object IntArrayOps extends ArrayOps[Int] with ArrayCreateOps[Int] {
    def create(length: Int): Array[Int] = new Array[Int](length)
    def get(array: Array[Int], index: Int): Int = array(index)
    def set(array: Array[Int], index: Int, value: Int): Unit = array(index) = value
  }

  object LongArrayOps extends ArrayOps[scala.Long] with ArrayCreateOps[scala.Long] {
    def create(length: Int): Array[scala.Long] = new Array[scala.Long](length)
    def get(array: Array[scala.Long], index: Int): scala.Long = array(index)
    def set(array: Array[scala.Long], index: Int, value: scala.Long): Unit = array(index) = value
  }
