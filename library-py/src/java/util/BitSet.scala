...
private object BitSet {
  private final val AddressBitsPerWord = 5
  ...
  def valueOf(lb: LongBuffer): BitSet = {
    ...
    lb.get(arr)
    lb.position(lb.position() - arr.length)
    valueOf(arr)
  }
...
class BitSet private (private var bits: Array[Int]) extends Serializable with Cloneable {
  ...
  final private def ensureLength(requiredLength: Int): Unit = {
    if (requiredLength > bits.length)
      bits = Arrays.copyOf(bits, Math.max(requiredLength, bits.length * 2))
  }
