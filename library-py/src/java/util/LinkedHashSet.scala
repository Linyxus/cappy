...
class LinkedHashSet[E] private[util] (inner: LinkedHashMap[E, AnyRef])
    extends AbstractSet[E] with Set[E] with Cloneable with Serializable:

  private val innerKeySet = inner.keySet()

  def this(initialCapacity: Int, loadFactor: Float) =
    this(new LinkedHashMap[E, AnyRef](initialCapacity, loadFactor))

  def this(initialCapacity: Int) =
    this(new LinkedHashMap[E, AnyRef](initialCapacity))

  def this() =
    this(new LinkedHashMap[E, AnyRef]())
  ...
  override def add(value: E): Boolean =
    inner.put(value, LinkedHashSet.Present) == null
