...
class DataInputStream(in: InputStream) extends FilterInputStream(in) with DataInput {
  ...
  private def eof(): Nothing =
    throw new EOFException()

  private def pushBack(v: Int): Unit =
    pushedBack = v
  ...
  def readUTF(): String = {
    ...
    def badFormat(msg: String): Nothing =
    ...
    while (i < length) {
      ...
      if ((a & 0x80) == 0x00) {
        ...
        res += Character.toString(((a & 0x1f) << 6) | (b & 0x3f))
        ...
        res += Character.toString(((a & 0x0f) << 12) | ((b & 0x3f) << 6) | (c & 0x3f))
      } else {
        val rem = length - i
